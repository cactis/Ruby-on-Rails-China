...
让我们再回来看看 `_form.html.erb` ，在 `<%= form_for(@girl) do |f| %>` 与
`<% end %>` 之间的区块：
...
这柳暗花明又一村呢，是哪一村，让我们看看 `config/routes.rb`
这一村（没有注解）：
...
这个表单的 `action` 指向 `/girls/2` ，也就是 `GirlsController` 的 `show`
...
这把你数据库的纪录永远地删除了。然后把你重定向至 `girls_url` 也就是这里
